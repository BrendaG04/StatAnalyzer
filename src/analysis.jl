...
function cramers_v(x, y)
    pairs = [(string(x[k]), string(y[k])) for k in eachindex(x)
             if !ismissing(x[k]) && !ismissing(y[k])]
    n = length(pairs)
    n == 0 && return (0.0, 0)

    x_levels = unique(first.(pairs))
    y_levels = unique(last.(pairs))
    rx = length(x_levels)
    cy = length(y_levels)
    (rx < 2 || cy < 2) && return (0.0, n)

    x_idx = Dict(v => i for (i, v) in enumerate(x_levels))
    y_idx = Dict(v => i for (i, v) in enumerate(y_levels))
    obs = zeros(Float64, rx, cy)

    for (vx, vy) in pairs
        obs[x_idx[vx], y_idx[vy]] += 1
    end

    row_sum = sum(obs, dims=2)
    col_sum = sum(obs, dims=1)
    total = sum(obs)
    total == 0 && return (0.0, n)

    expected = row_sum * col_sum / total
    chi2 = sum(((obs .- expected) .^ 2) ./ (expected .+ eps()))
    denom = min(rx - 1, cy - 1)
    denom <= 0 && return (0.0, n)

    v = sqrt((chi2 / total) / denom)
    return (round(v; digits=3), n)
end

function correlation_ratio(cat, num)
    pairs = [(string(cat[k]), Float64(num[k])) for k in eachindex(cat)
             if !ismissing(cat[k]) && !ismissing(num[k])]
    n = length(pairs)
    n == 0 && return (0.0, 0.0, 0)
    ...
    length(groups) < 2 && return (0.0, 0.0, n)

    all_vals = [v for (_, vals) in groups for v in vals]
    grand_mean = mean(all_vals)

    ss_between = 0.0
    for vals in values(groups)
        ss_between += length(vals) * (mean(vals) - grand_mean)^2
    end

    ss_total = sum((v - grand_mean)^2 for v in all_vals)
    ss_total <= eps() && return (0.0, 0.0, n)

    eta2 = ss_between / ss_total
    eta = sqrt(eta2)
    return (round(eta; digits=3), round(eta2; digits=3), n)
end
...
function run_analysis(filepath::String)
    ...
    if length(ccols) >= 2
        n = length(ccols)
        for i in 1:n, j in (i+1):n
            v, pair_n = cramers_v(df[!, ccols[i]], df[!, ccols[j]])
            push!(cat_cor_rows, Dict(
                "a" => ccols[i],
                "b" => ccols[j],
                "v" => v,
                "n" => pair_n,
                "strength" => v >= 0.5 ? "Strong" : v >= 0.3 ? "Moderate" : "Weak"
            ))
        end
    end

    mixed_cor_rows = []
    if !isempty(ccols) && !isempty(corr_cols)
        for c in ccols
            for ncol in corr_cols
                eta, eta2, pair_n = correlation_ratio(df[!, c], df[!, ncol])
                push!(mixed_cor_rows, Dict(
                    "cat" => c,
                    "num" => ncol,
                    "eta" => eta,
                    "eta2" => eta2,
                    "n" => pair_n,
                    "strength" => eta >= 0.5 ? "Strong" : eta >= 0.3 ? "Moderate" : "Weak"
                ))
            end
        end
    end
    ...
    Dict(
        ...
        "cat_stats" => cat_stats,
        "cor_rows" => cor_rows,
        "cat_cor_rows" => cat_cor_rows,
        "mixed_cor_rows" => mixed_cor_rows,
        "plots" => plots,
